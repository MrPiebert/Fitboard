...
document.addEventListener('DOMContentLoaded', () => {
    ...
    Fetch_Programs(); // Executes function on load
});

// Loads program names from the database into the 
...
function Fetch_Program_Details() { // Loads details of the selected program into the table container

    console.log("Fetching program details"); // Debug statement

    const programId = document.getElementById('Select_Program').value;

    if (programId === "Empty Program"){ // Unique case where empty program option is picked, no options to rename or delete
    
        const Table_Container = document.getElementById("Table_Container");
        Table_Container.innerHTML = "";
        const Empty_Table = Create_New_Day_Table();
        Table_Container.appendChild(Empty_Table);

    } else {    
        fetch(`Program Detail Fetching.php?program_id=${programId}`)
        .then(response => response.json())  // Parse the JSON from the response
        .then(data => {
            console.log(data);  // Log the data for debugging
            Load_Database_Program(data);  // Call your function to process the data
        })
